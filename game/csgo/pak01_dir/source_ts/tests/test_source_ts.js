...
function TEST_EQ(sTestName, actual, expected) {
    if (actual === expected) {
        Instance.Msg(`Test Passed: ${sTestName}`);
    }
    else {
        Instance.Msg(`Test FAILED: ${sTestName}`);
        Instance.Msg(`\texpected "${actual}" to equal ${expected}`);
    }
}
// ----------------------
...
// ----------------------
{
    ...
    if (ent_a && ent_b) {
        ...
        Instance.Msg("FAILED to create entities");
    }
    if (ent_a)
...
TEST_EQ("bDidThrow", bDidThrow, true); // Expect Domain methods to require a proper this value
